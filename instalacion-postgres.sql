...
*************************************************************************************************************************************************/
	...
	*/
	...
	comment on table STOC_ARTICULO is 'Maestro de articulos';
	comment on column STOC_ARTICULO.ARTS_ARTICULO is 'Clave principal que identifica a un articulo';
	comment on column STOC_ARTICULO.ARTS_NOMBRE is 'Nombre del articulo, se recomienda <nombre> <formato> <unidad medida>';
	comment on column STOC_ARTICULO.CANTIDAD_MINIMA is 'Indica la cantidad minima que debe haber en el stock antes de emitir una notificacion';
	comment on column STOC_ARTICULO.PROV_NOTAS is 'Notas, comentarios y observaciones sobre el articulo';
	comment on column STOC_ARTICULO.PROV_BAJA is 'Indica si el articulo esta dado de baja [true] o no [false]';
	
		CREATE OR REPLACE FUNCTION STOC_ARTICULO_auditoria_insert_func() RETURNS TRIGGER
		AS $STOC_ARTICULO_auditoria_insert$
		BEGIN
			insert into AUDI_AUDITORIA (
				AUDI_OPERACION,
				AUDI_MODELO,
			...
			RETURN NEW;
		END;
		$STOC_ARTICULO_auditoria_insert$ language plpgsql;
		
		CREATE OR REPLACE FUNCTION STOC_ARTICULO_auditoria_update_func() RETURNS TRIGGER
		AS $STOC_ARTICULO_auditoria_update$
		BEGIN
			insert into AUDI_AUDITORIA (
				AUDI_OPERACION,
				AUDI_MODULO,
			...
			RETURN NEW;
		END;
		$STOC_ARTICULO_auditoria_update$ language plpgsql;
		
		CREATE OR REPLACE FUNCTION STOC_ARTICULO_auditoria_delete_func() RETURNS TRIGGER
		AS $STOC_ARTICULO_auditoria_delete$
		BEGIN
			insert into AUDI_AUDITORIA (
				AUDI_OPERACION,
				AUDI_MODULO,
			...
			RETURN NEW;
		END;
		$STOC_ARTICULO_auditoria_delete$ language plpgsql;
		
		/*------------------------------------------
		Registro en auditoria nuevo articulo
		--------------------------------------------*/
		DROP TRIGGER IF EXISTS STOC_ARTICULOS_auditoria_insert ON STOC_ARTICULOS;
		CREATE TRIGGER STOC_ARTICULOS_auditoria_insert
		after insert on STOC_ARTICULOS
		for each row EXECUTE PROCEDURE STOC_ARTICULO_auditoria_insert_func();
		
		/*------------------------------------------
		Registro en auditoria actualizacion de articulo
		--------------------------------------------*/
		DROP TRIGGER IF EXISTS STOC_ARTICULOS_auditoria_update ON STOC_ARTICULOS;
		CREATE TRIGGER STOC_ARTICULOS_auditoria_update
		after update on STOC_ARTICULOS
		for each row EXECUTE PROCEDURE STOC_ARTICULO_auditoria_update_func();
		
		/*------------------------------------------
		Registro en auditoria eliminacion de articulo
		--------------------------------------------*/
		DROP TRIGGER IF EXISTS STOC_ARTICULOS_auditoria_delete ON STOC_ARTICULOS;
		CREATE TRIGGER STOC_ARTICULOS_auditoria_delete
		before delete on STOC_ARTICULOS
		for each row EXECUTE PROCEDURE STOC_ARTICULO_auditoria_delete_func();
